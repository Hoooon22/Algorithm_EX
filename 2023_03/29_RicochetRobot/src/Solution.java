...
public class Solution {
    ...
    public static int solution(String[] board) {
        ...
        answer = bfs(arr_board, start[0], start[1]);

        return answer;
    }

    public static int bfs(char[][] arr_board, int x, int y) {
        int goal = 0;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < dx.length; i++) {
                ...
                // 다음 방향이 바로 벽이 아니라면, 벽에 부딪힐때 까지 전진
                if (arr_board[nx+dx[i]][ny+dy[i]] != 'D') {
                    for (int j = 1; j < 100; j++) {
                        ...
                        else if (arr_board[nx+dx[i]][ny+dy[i]] == 'D' && !visited[nx][ny]) { // 현재 기준, 다음 블럭이 벽이면서 방문 안했다면
                            // 현재 위치를 큐에 저장
                            visited[nx][ny] = true;
                            queue.add(new int[]{nx, ny});
                            break;
                        }
                        else if (arr_board[nx+dx[i]][ny+dy[i]] == 'D' && visited[nx][ny]) { // 벽인데 방문했었다면,
                            // 스킵
                            break;
                        }
                        // 나머지는 다음으로 전진
